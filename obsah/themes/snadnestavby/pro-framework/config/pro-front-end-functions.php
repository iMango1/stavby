...
if ( ! function_exists( 'webuild_full_top_meta' ) ) {
	function webuild_full_top_meta( $post ) {
		...
		// details
		$id = get_the_author_meta( 'ID' );
		echo '<span class="entry-profile"><span class="col"><span class="entry-author-link"><strong>' . esc_html__( 'By:', 'webuild' ) . '</strong><span class="author vcard"><a class="url fn n" href="' . esc_url(get_author_posts_url( $id )) . '" rel="author">' . get_the_author() . '</a></span></span><span class="entry-date"><strong>Posted: </strong>' . esc_html( get_the_date( 'M jS, Y' ) ) . '</span></span></span>';
		// comments
		...
		if ( ! is_search() ) {
			if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
				...
				comments_popup_link( esc_html__( '0', 'webuild' ), esc_html__( '1', 'webuild' ), esc_html__( '%', 'webuild' ) );
				echo '</span>';
			}
		}
		echo '</header>';
...
if ( ! function_exists( 'webuild_medium_type_meta' ) ) {
	function webuild_medium_type_meta() {
		...
		echo '<span class="entry-author-link">' . esc_html__( 'By:', 'webuild' ) . '<span class="author vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '" rel="author">' . get_the_author() . '</a></span></span>';
		$post_format = get_post_format();
		if ( $post_format ) {
			echo '<span class="entry-format-' . $post_format . '">';
...
if ( ! function_exists( 'webuild_posted_on' ) ) {
	function webuild_posted_on() {
		...
		if ( is_sticky() && is_home() && ! is_paged() ) {
			echo '<span class="entry-featured">' . esc_html__( 'Sticky', 'webuild' ) . '</span>';
		}
		$post_format = get_post_format();
		if ( $post_format ) {
			echo '<span class="entry-format-' . $post_format . '">';
		...
		if ( ! is_search() ) {
			if ( ! post_password_required() && ('open' == $post->comment_status) ) {
				...
				esc_html_e( 'Comments are Off', 'webuild' );
				echo '</a></span>';
			}
			$live_id = get_the_ID();
		...
		edit_post_link( esc_html__( 'Edit', 'webuild' ), '<span class="entry-edit-link">', '</span>' );
	}
}
/**
 *
...
if ( ! function_exists( 'webuild_post_nav' ) ) {
	function webuild_post_nav() {
		...
		<nav class="post-navigation">
			<h3 class="nav-previous"><?php previous_post_link( '%link', '<i class="fa fa-angle-left"></i> ' . esc_html__( 'Previous Post', 'webuild' ) . '<br /><strong><p>%title</p></strong>' ); ?></h3>

			<h3 class="nav-next"><?php next_post_link( '%link', esc_html__( 'Next Post', 'webuild' ) . ' <i class="fa fa-angle-right"></i><br /><strong><p>%title</p></strong>' ); ?></h3>
			<div class="clear"></div>
		</nav>
		<?php
	}
...
if ( ! function_exists( 'webuild_link_pages' ) ) {
	function webuild_link_pages( $args = '' ) {
		$defaults = array(
			'title'            => '<span class="pagination-title">' . esc_html__( 'Pages', 'webuild' ) . '</span>',
			...
			'separator'        => '',
			'nextpagelink'     => esc_html__( 'Next', 'webuild' ),
			'previouspagelink' => esc_html__( 'Previous', 'webuild' ),
			'pagelink'         => '%',
			'echo'             => 1
		);
...
if ( ! function_exists( 'webuild_paging_nav' ) ) {
	function webuild_paging_nav( $args = array() ) {
		...
		if ( $args['nav'] == 'load' ) {
			...
			$output .= esc_html__( 'Load More', 'webuild' );
			...
			$links = paginate_links( array(
				...
				'prev_text' => esc_html__( 'Previous', 'webuild' ),
				'next_text' => esc_html__( 'Next', 'webuild' ),
			) );
			if ( $links ) {
				?>
				<div class="clear"></div>
 ...
 *
 */
if ( ! function_exists( 'webuild_portfolio_item' ) ) {
	function webuild_portfolio_item( $item_args ) {
		global $post;
		$post_id = get_the_ID();
		...
		switch ( $comment->comment_type ) :
			...
			case 'trackback' :
				...
				<div class="pingback-content">
					<span><?php esc_html_e( 'Pingback:', 'webuild' ); ?></span>
					<?php comment_author_link(); ?>

					<?php edit_comment_link( esc_html__( '(Edit)', 'webuild' ), ' ' ); ?>
				</div>
				<?php
				break;
			default :
				...
				<article id="div-comment-<?php comment_ID(); ?>">
					...
					<div class="comment-meta">
						<time datetime="<?php comment_time( 'c' ); ?>">
							...
							printf( esc_html__( '%1$s at %2$s', 'webuild' ), get_comment_date(), get_comment_time() ); ?>
						</time>
						<?php edit_comment_link( esc_html__( '(Edit)', 'webuild' ), ' ' ); ?>
					</div>
					<!-- .comment-meta -->
					...
					<?php if ( $comment->comment_approved == '0' ) : ?>
						<em><?php esc_html_e( 'Your comment is awaiting moderation.', 'webuild' ); ?></em>
						<br/>
					<?php endif; ?>
