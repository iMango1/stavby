...
if ( ! function_exists( 'webuild_after_setup_theme' ) ) {
	function webuild_after_setup_theme() {
		...
		register_nav_menus( array(
			'primary'     => esc_html__( 'Primary Menu', 'webuild' ),
			'top_menu'    => esc_html__( 'Top Menu', 'webuild' ),
			'one_page'    => esc_html__( 'One Page Menu', 'webuild' ),
			'footer_menu' => esc_html__( 'Footer Menu', 'webuild' ),
		) );
		load_theme_textdomain( 'webuild', THEME_DIR . '/languages' );
	}
...
if ( ! function_exists( 'webuild_display_post_author' ) ) :
	function webuild_display_post_author() {
		...
		<div class="pro-fancy-separator title-left fancy-author-title">
			<h5 class="pro-fancy-title"><?php esc_html_e( 'About the author', 'webuild' ); ?><span
 ...
 *
 */
if ( ! function_exists( 'webuild_import_dump' ) ) {
	function webuild_import_dump() {
		echo '<div id="pro-install-result">';
		//
		// importing xml
		// -----------------------------------------------------------------------
		...
		if ( ! empty( $menu_items ) ) {
			$menu_fields = array(
				...
				'Home v3'                           => array(
					'highlight'      => 'Transparent Menu',
					'highlight_type' => 'warning'
				),
				'Home v5 - Logo Center Transparent' => array(
		...
		update_option( 'page_on_front', webuild_get_id_by_slug( 'home-v1-2' ) );
		global $reduxConfig;
		$file          = FRAMEWORK_DIR . '/config/dump/redux.json';
		$file_contents = file_get_contents( $file );
		$options       = json_decode( $file_contents, true );
		$data          = get_option( 'apro_options' );
		foreach ( $options as $key => $value ) {
			$data[ $key ] = $value;
			//$reduxConfig->ReduxFramework->set( $key, $value );
		}
		update_option( 'apro_options', $data );
		echo '</div>';
		if ( strpos( $wp_import_result, 'All done' ) !== false ) {
			update_option( 'pro-installed', true );
		}
		die();
	}

	add_action( 'wp_ajax_pro-import-dump', 'webuild_import_dump' );
}
/**
 *
