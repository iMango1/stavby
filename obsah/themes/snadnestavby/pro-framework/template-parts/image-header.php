									...
									<h1>
										...
										if ( $apro_options['custom-title-switch'] ) {
											...
											elseif ( is_tag() ) :
												single_tag_title();
											elseif ( is_author() ) :
												printf( esc_html__( 'Author: %s', 'webuild' ), '<span class="vcard">' . get_the_author() . '</span>' );
											elseif ( is_day() ) :
												printf( esc_html__( 'Day: %s', 'webuild' ), '<span>' . get_the_date() . '</span>' );
											elseif ( is_month() ) :
												printf( esc_html__( 'Month: %s', 'webuild' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'webuild' ) ) . '</span>' );
											elseif ( is_year() ) :
												printf( esc_html__( 'Year: %s', 'webuild' ), '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'webuild' ) ) . '</span>' );
											elseif ( is_tax( 'post_format', 'post-format-aside' ) ) :
												esc_html_e( 'Asides', 'webuild' );
											elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) :
												esc_html_e( 'Galleries', 'webuild' );
											elseif ( is_tax( 'post_format', 'post-format-image' ) ) :
												esc_html_e( 'Images', 'webuild' );
											elseif ( is_tax( 'post_format', 'post-format-video' ) ) :
												esc_html_e( 'Videos', 'webuild' );
											elseif ( is_tax( 'post_format', 'post-format-quote' ) ) :
												esc_html_e( 'Quotes', 'webuild' );
											elseif ( is_tax( 'post_format', 'post-format-link' ) ) :
												esc_html_e( 'Links', 'webuild' );
											elseif ( is_tax( 'post_format', 'post-format-status' ) ) :
												esc_html_e( 'Statuses', 'webuild' );
											elseif ( is_tax( 'post_format', 'post-format-audio' ) ) :
												esc_html_e( 'Audios', 'webuild' );
											elseif ( is_tax( 'post_format', 'post-format-chat' ) ) :
												esc_html_e( 'Chats', 'webuild' );
											elseif ( is_search() ) :
												printf( esc_html__( 'Search Results for: %s', 'webuild' ), get_search_query() );
											elseif ( is_woocommerce_shop() ) :
												$webuild_post_id = wc_get_page_id( 'shop' );
												echo get_the_title( $webuild_post_id );
											elseif ( is_archive() ) :
												esc_html_e( 'Archives', 'webuild' );
											else :
												the_title();
											endif;
										}; ?>
