 ...
 *
 */
function webuild_team( $atts, $content = '' ) {
	global $webuild_team_columns;
	...
	//$webuild_team_columns = $columns;
	$regex = webuild_get_shortcode_regex( 'webuild_team_member' );
	// count columns
	preg_match_all( '/' . $regex . '/', $content, $count_list );
	$webuild_team_columns = count( $count_list[0] );
	// begin output
	$output = '';
	$output .= '<div' . $id . ' class="pro-team' . $class . '"' . $in_style . '>';
...
add_shortcode( 'webuild_team', 'webuild_team' );
/**
 *
 * PRO Team Member
 * @version 1.0.0
 ...
 *
 */
function webuild_team_member( $atts, $content = '' ) {
	global $webuild_team_columns;
	extract( shortcode_atts( array(
		'image'        => '',
		'name'         => '',
	...
	$output = '';
	$output .= '<div class="pro-team-member ' . webuild_get_bootstrap( $webuild_team_columns ) . '">';
	$output .= ( $image ) ? '<figure><span class="outline"></span><img src="' . $image . '" alt="' . $name . '" /></figure>' : '';
...
add_shortcode( 'webuild_team_member', 'webuild_team_member' );